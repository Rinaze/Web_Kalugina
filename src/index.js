...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    if (scoreElement) {
        scoreElement.innerHTML = snake.bodyLength;
    }
  },
};
...
const snake = {
  ...
  /**
   * Обновляет внутренний счетчик длины тела (без учета головы).
   */
  setBodyLength() {
    this.bodyLength = this.body.length - 1;
  },
   ...
   */
  makeStep() {
    // Получаем точку, где будет голова на следующем шаге.
    const nextStepPoint = this.getNextStepHeadPoint();
    // Добавляем новую голову в начало массива тела.
    this.body.unshift(nextStepPoint);
    // Удаляем последний элемент (хвост), чтобы змейка не росла бесконечно при движении.
    this.body.pop();
  ...
  getNextStepHeadPoint() {
    ...
    // Возвращаем точку, где окажется голова змейки в зависимости от направления и границ поля.
    switch (this.direction) {
      case "up":
        return { 
          x: firstPoint.x, 
          y: firstPoint.y <= 0 ? config.getRowsCount() - 1 : firstPoint.y - 1 
        };
      case "right":
        return { 
          x: firstPoint.x >= config.getColsCount() - 1 ? 0 : firstPoint.x + 1, 
          y: firstPoint.y 
        };
      case "down":
        return { 
          x: firstPoint.x, 
          y: firstPoint.y >= config.getRowsCount() - 1 ? 0 : firstPoint.y + 1 
        };
      case "left":
        return { 
          x: firstPoint.x <= 0 ? config.getColsCount() - 1 : firstPoint.x - 1, 
          y: firstPoint.y 
        };
    }
  },
...
/**
 * Объект игры.
 * @property {config} config Настройки игры.
...
const game = {
  ...
  getRandomFreeCoordinates() {
    while (true) {
      ...
      // Проверяем, не занята ли эта точка змейкой или едой.
      if (!this.snake.isOnPoint(rndPoint) && !this.food.isOnPoint(rndPoint)) {
        return rndPoint;
      }
    }
  },
  ...
  canMakeStep() {
    ...
    // Шаг невозможен, если следующая точка — это тело самой змейки.
